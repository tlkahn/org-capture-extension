  ...
  class Capture {
    ...
    async getSelectionTxt() {
      ...
      return new Promise(async (resolve, reject) => {
        ...
        this.selection_html += `\n${this.location.href}`;
        console.log(
          this.stripInvalidFileNameChars(this.document.title + '.org')
        );
        let filename_main = this.document.title
          ? this.document.title
          : document.location.pathname.slice(1);
        let filename = this.stripInvalidFileNameChars(filename_main + '.org');
